...
export const PublicLanding: React.FC<PublicLandingProps> = ({ onRegister, onLogin, onViewGuide, isProcessing }) => {
  ...
  const demoIcons = [
    'https://image2url.com/r2/default/images/1770491023736-6e9ee944-903d-4f53-9db8-9d7945ec8f35.png', // TokenPod Primary Logo
    'https://is1-ssl.mzstatic.com/image/thumb/Purple221/v4/31/45/ae/3145ae77-0c87-5456-00fe-656c1f6f925c/AppIcon-0-0-1x_U007emarketing-0-8-0-85-220.png/200x200ia-75.webp', // Pi
    'https://is1-ssl.mzstatic.com/image/thumb/Purple211/v4/91/7a/20/917a206a-9a9f-8557-017e-97621980004c/AppIcon-0-0-1x_U007emarketing-0-0-0-10-0-0-sRGB-0-0-0-GLES2_U002c0-512MB-85-220-0-0.png/512x512bb.png', // Bee
    'https://is1-ssl.mzstatic.com/image/thumb/Purple221/v4/64/0e/96/640e9603-9112-9c32-1594-555621455219/AppIcon-0-0-1x_U007emarketing-0-6-0-0-85-220.png/512x512bb.png', // Xenea
  ];

  // 1. Fetch Real Community Data from Shards
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-orange-500/30">
      ...
      <section className="relative pt-20 pb-16 px-8 text-center overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-orange-600/5 blur-[120px] rounded-full -z-10" />
        ...
        {/* Enhanced Hyper-Sync Demo Card */}
        <div className="max-w-xs mx-auto mb-16 animate-in zoom-in duration-500 delay-300">
          ...
          <p className="mt-4 text-[8px] font-black text-slate-600 uppercase tracking-widest">Experience the Universal Harvest Protocol</p>
        </div>
      ...
      {/* Universal Compatibility Grid */}
      <section className="px-6 mb-24">
        <div className="text-center mb-10">
          <h2 className="text-[11px] font-black uppercase tracking-[0.3em] text-orange-500 mb-2">Universal Reach</h2>
          <p className="text-2xl font-black uppercase tracking-tighter">Works with <span className="text-slate-400">Any App.</span></p>
        </div>

        <div className="grid grid-cols-2 gap-4 mb-8">
           <div className="bg-white/5 border border-white/10 rounded-3xl p-6 text-center">
              <MessageSquare className="mx-auto mb-3 text-blue-400" size={24} aria-hidden="true" />
           ...
           <div className="bg-white/5 border border-white/10 rounded-3xl p-6 text-center">
              <LayoutGrid className="mx-auto mb-3 text-emerald-400" size={24} aria-hidden="true" />
           ...
           <div className="bg-white/5 border border-white/10 rounded-3xl p-6 text-center">
              <Smartphone className="mx-auto mb-3 text-purple-400" size={24} aria-hidden="true" />
              <h4 className="text-[10px] font-black uppercase tracking-tight text-white">Mobile Apps</h4>
              <p className="text-[8px] font-bold text-slate-500 uppercase mt-1">Pi, Bee, Ice, Avive</p>
           </div>
           <div className="bg-white/5 border border-white/10 rounded-3xl p-6 text-center">
              <Zap className="mx-auto mb-3 text-orange-400" size={24} aria-hidden="true" />
              <h4 className="text-[10px] font-black uppercase tracking-tight text-white">Daily Rewards</h4>
              <p className="text-[8px] font-bold text-slate-500 uppercase mt-1">Streaks & Check-ins</p>
           </div>
        </div>
        ...
        <div className="bg-slate-900/40 border border-white/5 rounded-[2.5rem] overflow-hidden divide-y divide-white/5 shadow-2xl">
          <div className="p-4 bg-white/5 flex items-center justify-center gap-2">
            ...
            <span className="text-[8px] font-black text-orange-500 uppercase tracking-widest">Global Community Signal Feed</span>
          </div>
          
          {isLoadingSignals ? (
            <div className="p-20 flex flex-col items-center justify-center gap-4">
              <Loader2 className="animate-spin text-slate-700" size={32} />
              <p className="text-[8px] font-black text-slate-700 uppercase tracking-widest">Scanning Shards...</p>
            </div>
          ) : (
            liveSignals.map((sig) => {
              const iconUrl = enhancedIcons[sig.id];
              const isPlaceholder = !iconUrl;

              return (
                <div key={sig.id} className="p-5 flex items-center gap-5 group hover:bg-white/5 transition-colors">
      ...
      <section className="px-8 space-y-24 mb-32">
        <div className="text-center">
          <Shield className="text-orange-500 mb-6 mx-auto" size={40} aria-hidden="true" />
          <h2 className="text-2xl font-black uppercase tracking-tighter mb-8">Our Privacy Rules</h2>
          <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 space-y-8">
            <div className="grid grid-cols-1 gap-6">
               <div className="flex items-center gap-4 text-left">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-tight">No Email Required</p>
               </div>
               <div className="flex items-center gap-4 text-left">
                  <div className="w-10 h-10 rounded-xl bg-orange-500/10 flex items-center justify-center text-orange-500 shrink-0"><Check size={20} /></div>
                  <p className="text-[10px] font-black uppercase tracking-tight">No Private Keys or Seed Phrases</p>
               </div>
               <div className="flex items-center gap-4 text-left">
                  <div className="w-10 h-10 rounded-xl bg-orange-500/10 flex items-center justify-center text-orange-500 shrink-0"><Check size={20} /></div>
                  <p className="text-[10px] font-black uppercase tracking-tight">No Tracking Advertisements</p>
               </div>
            </div>
            <p className="text-[9px] font-bold text-slate-500 uppercase leading-relaxed text-left border-t border-white/5 pt-8">
              TokenPod is a local-first utility. Your data is synced securely to the cloud only to keep your points safe across devices.
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="setup" className="px-8 pb-40 animate-in slide-in-from-bottom duration-1000">
        ...
        <div className="bg-white/5 border border-white/10 rounded-[3rem] p-8 space-y-6 shadow-2xl relative">
          ...
          <div className="space-y-3">
            <label className="text-[9px] font-black text-slate-500 ml-4 tracking-widest uppercase">Invite Code (Optional)</label>
            <div className="relative">
              <Tag className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500" size={16} />
              <input 
                type="text" 
                value={refCode} 
                onChange={e => setRefCode(e.target.value.toUpperCase())} 
                placeholder="REF-CODE" 
                className="w-full bg-black border border-white/10 rounded-2xl py-5 pl-14 pr-6 outline-none focus:border-orange-500/30 text-[10px] font-bold text-white transition-all uppercase" 
              />
            </div>
          </div>
